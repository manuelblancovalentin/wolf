...
def prepare_native_orfs(
    ...
    *,
    ...
    orfs_root = orfs_root.expanduser().resolve()
    ...
    if base_config is None:
        base_config = generated / "base-config.mk"
        base_config.write_text(
            "# Generated by WOLF for an external RTL package.\n"
            f"export PLATFORM = {_make_value(context.process)}\n",
            encoding="utf-8",
        )
        base_config_source = "generated"
    ...
    if configured and base_config_source == "explicit" and not base_config.is_relative_to(orfs_root):
